...
pdfs <- list.files("enforcement/", recursive = T, include.dirs = F) |> 
  purrr::keep(.p = \(x) stringr::str_detect(x, pattern =  ".pdf"))

text <- map_chr(
  paste0("enforcement/", pdfs) 
...
readr::write_csv(final, "data/enf_citations.csv")
